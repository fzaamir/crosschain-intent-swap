...
const App = () => {
  ...
  const getExecutionTime = (expiry, minTime, maxTime) => {
    switch(expiry) {
      case '30 seconds':
  ...
  const handleNext = useCallback(() => {
    setState(prev => {
      ...
      if (newState.step === 1) {
        ...
        const executionTime = getExecutionTime(newState.intentData.expiry, 5000, 40000);
        setTimeout(() => {
          setState(prev => ({
            ...prev,
            step: 4
        ...
        const settlementTime = getSettlementTime(newState.intentData.expiry);
